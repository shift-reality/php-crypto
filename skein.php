...
abstract class SkeinBigCore {
    ...
    /*
      public function update0(/* byte * /$in) {
      if ($this->ptr === static::BLOCK_LEN) {
      $etype = ($this->bcount->lo == 0) ? 224 : 96; //int
      //++$this->bcount;
      $this->bcount->addOne();
      $this->ubi($etype, 0);
      $this->buf[0] = $in & 0xff;
      ...
      $this->buf[$this->ptr++] = $in & 0xff;
      }
      } */

    /** @see Digest */
    public function update1(array $inbuf) {
        $this->update($inbuf, 0, count($inbuf));
    }

    /** @see Digest */
    public function update2(array $inbuf, $off, $len) {
        if ($len <= 0)
            return;
        ...
        for (;;) {
            ...
            if ($len <= static::BLOCK_LEN)
                break;
            arraycopy($inbuf, $off, $this->buf, 0, static::BLOCK_LEN);
    ...
    /** @see Digest */
    public function digest1($inbuf) {
        $this->update2($inbuf, 0, count($inbuf));
        return $this->digest0();
    }

    /** @see Digest */
    public function digest2(array &$outbuf, $off, $len) {
     ...
     */
    abstract function getInitVal();

    private function encodeLELong(o_u64 $val, array &$buf, $off) {
    ...
    private function decodeLELong(array $buf, $off) {
    ...
    private function ubi(/* int */$etype, /* int */ $extra) {
        ...
        $t1 = $this->bcount->shiftRightUnsigned(58)->plus($etype_u64->shiftLeft(55));
        $t2 = $t0->__xor($t1);
...
class Skein256 extends SkeinBigCore {
    ...
    /** @see Digest */
    public function getDigestLength() {
        return 32;
    }
...
/*
  $o = new o_u64(0xf25d7511, 0x6d28cd98);
  $t = new o_u64(0x65c41a8f, 0x2bdee5f5);

  var_dump((string) $o->shiftRightUnsigned(15));
  #var_dump((string) $o->shiftRightUnsigned(15));

  var_dump((string) $o->shiftRightUnsigned(55));
  #var_dump((string) $o->shiftRightUnsigned(55));
